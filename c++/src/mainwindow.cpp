 ...
 * Tương đương DSAController.java trong project Java
 *
...
// ============================================================
// Load QSS stylesheet từ Qt resource
// ============================================================
...
// Helper: button with object name for QSS
...
QWidget* MainWindow::setupTabKeyGen() {
    ...
    progressBar_->setRange(0, 0); // Indeterminate (spinning)
...
// ============================================================
// SLOT: Sinh Khóa (Tab 1)
// Chạy trên QThread để không block UI
// ============================================================
...
// ============================================================
// SLOT: Sinh khóa xong (callback từ QThread)
// ============================================================
void MainWindow::onKeyGenFinished(bool success) {
    ...
    // Hiện kết quả với fade-in animation
...
// ============================================================
// SLOT: Tải thông điệp từ file text
// ============================================================
...
void MainWindow::onAutoFillVerify() {
    ...
    tabWidget_->setCurrentIndex(2); // Chuyển sang Tab 3
}

// ============================================================
// SLOT: Xác minh chữ ký (Tab 3)
// ============================================================
void MainWindow::onVerifySignature() {
    ...
    if (message.isEmpty()) {
        setStatus(labelVerifyStatus_, "⚠️ Vui lòng nhập thông điệp cần xác minh!", "#F0AD4E");
        return;
    }
    if (rHex.isEmpty() || sHex.isEmpty()) {
        setStatus(labelVerifyStatus_, "⚠️ Vui lòng nhập cả r và s!", "#F0AD4E");
        return;
    }
    if (!model_->hasKeyPair()) {
        setStatus(labelVerifyStatus_, "⚠️ Chưa có khóa công khai! Vui lòng sinh khóa ở Tab 1.", "#F0AD4E");
        return;
    }
    ...
    // Hiển thị giá trị trung gian
    // Cắt ngắn để dễ đọc (chỉ hiện 32 ký tự đầu + "...")
    ...
    if (valid) {
        ...
        setStatus(labelVerifyStatus_, "✅ Xác minh thành công! Thông điệp chưa bị thay đổi và đúng nguồn gốc.", "#5CB85C");
    } else {
        ...
        verifyCircle_->style()->unpolish(verifyCircle_);
        verifyCircle_->style()->polish(verifyCircle_);
        setStatus(labelVerifyStatus_, "⚠️ Cảnh báo: Thông điệp có thể đã bị giả mạo, hoặc chữ ký / khóa không khớp!", "#D9534F");
    }
